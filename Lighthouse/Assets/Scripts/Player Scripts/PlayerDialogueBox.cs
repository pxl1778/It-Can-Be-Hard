...
public class PlayerDialogueBox : MonoBehaviour {
    ...
    void Update()
    {
        ...
        Vector3 targetPos = new Vector3(target.position.x, target.position.y + (1.0f * canvasRect.localScale.x), target.position.z);
        Vector2 viewportPoint = cam.WorldToViewportPoint(targetPos);
        Vector2 desiredPoint = new Vector3(viewportPoint.x * canvasRect.sizeDelta.x, (viewportPoint.y + 0.5f) * canvasRect.sizeDelta.y, 0) * canvasRect.localScale.x;
